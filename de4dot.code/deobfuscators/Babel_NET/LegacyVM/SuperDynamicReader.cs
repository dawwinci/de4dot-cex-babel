...
namespace de4dot.code.deobfuscators.Babel_NET.LegacyVM
{
	public class SuperDynamicReader : MethodBodyReaderBase, ISignatureReaderHelper
	{
		...
		public SuperDynamicReader(ModuleDef module, object obj, GenericParamContext gpContext)
		{
			...
			Delegate delegate1 = obj as Delegate;
			if (delegate1 != null)
			{
				obj = delegate1.Method;
				if (obj == null)
				{
			...
			this.reader = MemoryImageStream.Create(array);// ByteArrayDataReaderFactory.CreateReader(array);//MemoryImageStream.Create(array);
		...
		private void SecondOption(object obj)
		{
			...
			this.reader =  MemoryImageStream.Create(fieldValue);//ByteArrayDataReaderFactory.CreateReader(fieldValue); // MemoryImageStream.Create(fieldValue); //MemoryImageStream.Create(fieldValue);
		...
		{
			MethodInfo result;
			if (SuperDynamicReader.vamDynamicMethodFieldInfo.Exists(obj))
			{
        ...
        //https://stackoverflow.com/questions/26972066/type-from-intptr-handle
        ...
        TypeSig ISignatureReaderHelper.ConvertRTInternalAddress(IntPtr address)
		{
			...
			/*try
			{
                resolved1 = MethodTableToTypeConverter.Convert(address);
			}
			catch
			{*/
                resolved1 = GetTypeFromHandle(address);
            //}
				//if (resolved1==null)
				//{
				
				//Console.WriteLine(resolved1.FullName);
				//Console.ReadKey();
            //}

            return this.importer.ImportAsTypeSig(resolved1);
		}
		...
		private static readonly SuperDynamicReader.ReflectionFieldInfo dmResolverFieldInfo = new SuperDynamicReader.ReflectionFieldInfo("m_resolver", "_resolver");
		...
		private static readonly SuperDynamicReader.ReflectionFieldInfo methodDynamicInfo = new SuperDynamicReader.ReflectionFieldInfo("m_DynamicILInfo", "_dynamicILInfo");
